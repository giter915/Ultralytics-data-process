...
if __name__ == '__main__':
    labelme_path = "./maskdataset"
    saved_coco_path = "./coco-formatcoco1"
    print('reading...')
    # 创建文件
    ...
    # 数据划分为训练集、验证集和测试集
    train_val_path, test_path = train_test_split(json_list_path, test_size=0.1, train_size=0.9, random_state=42)
    train_path, val_path = train_test_split(train_val_path, test_size=0.1, train_size=0.9, random_state=42)
    print("train_n:", len(train_path), 'val_n:', len(val_path), 'test_n:', len(test_path))
    ...
    for file in test_path:
        shutil.copy(file.replace("json", "jpg"), "%scoco/images/test2017/" % saved_coco_path)
        img_name = file.replace('json', 'jpg')
        temp_img = cv2.imread(img_name)
        try:
            cv2.imwrite(
                "{}coco/images/test2017/{}".format(saved_coco_path, img_name.split('\\')[-1].replace('png', 'jpg')),
                temp_img)
        except Exception as e:
            print(e)
            print('Wrong Image:', img_name)
